    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <Card className="w-full max-w-sm border-rose-500/30">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">
              El activo quedará disponible de nuevo para otros usuarios. Esta acción no se puede deshacer.
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            className="flex-1 bg-rose-600 hover:bg-rose-500 text-white border-0 font-bold"
            onClick={onConfirm}
          >
            Sí, cancelar
          </Button>
...
function MyLoansView({ onShowQR, onFeedback }: {
  ...
  // Para obtener el request real (no agrupado) para el modal de detalle
  const getRealRequest = (req: Request): Request => {
    if (req.bundle_group_id) {
      // Devolver el primer request del bundle con toda la info
  ...
  return (
    <div className="space-y-6">
      ...
      {cancelReqId !== null && (
        <CancelConfirmModal
          onConfirm={() => handleCancel(cancelReqId)}
        ...
        <div className="space-y-3">
          ...
          {active.map(req => {
            ...
            return (
              <Card
                key={req.id}
                className={`border transition-all ${isOverdue ? 'border-rose-500/40' : isActionRequired ? 'border-orange-500/40' : 'border-white/5'}`}
              >
                <div className="flex items-center justify-between gap-4">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-600 mt-2 flex items-center gap-1">
                      <Info size={9} /> Toca para ver detalles completos
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0 items-end">
                    ...
                    {isActionRequired && (
                      <Button
                        ...
                        className="text-[11px] h-8 border-orange-500/40 text-orange-400"
                      >
                        <MessageSquare size={12} className="mr-1" /> Responder
                      </Button>
                    )}
                    {/* ── BOTÓN CANCELAR: solo si está PENDING o ACTION_REQUIRED ── */}
                    {isCancellable && (
                      <button
...
export function UserHome({ isManagerView = false, onBack }: { isManagerView?: boolean; onBack?: () => void }) {
  ...
  // Solo mostrar activos DISPONIBLES (En trámite, Prestada, etc. quedan excluidos)
    ...
    <div className={`min-h-screen ${isManagerView ? 'bg-transparent' : 'bg-background'} font-sans pb-24`}>
      ...
      <main className="p-4">
        {activeTab === 'catalog' ? (
          ...
          <MyLoansView
            onShowQR={(req) => setQRRequest(req)}
            onFeedback={(req) => setFeedbackRequest(req)}
          />
        )}
      </main>
