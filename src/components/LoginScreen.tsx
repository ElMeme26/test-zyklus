      ...
      <Card className="w-full max-w-sm bg-slate-900/90 login-card border-slate-800 relative z-10 backdrop-blur-2xl shadow-2xl">
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
           ...
           <div className="grid grid-cols-2 gap-2 text-[10px]">
              <button onClick={() => handleQuickLogin('sara.gomez@zf.com')} className="p-2 bg-slate-950 border border-slate-800 rounded hover:border-cyan-500 text-slate-400 hover:text-cyan-400 transition-colors flex items-center justify-center gap-2">
                👷‍♂️ Usuario
              </button>
              <button onClick={() => handleQuickLogin('lic.echeverria@zf.com')} className="p-2 bg-slate-950 border border-slate-800 rounded hover:border-cyan-500 text-slate-400 hover:text-cyan-400 transition-colors flex items-center justify-center gap-2">
                👔 Líder
              </button>
              <button onClick={() => handleQuickLogin('juana.pichardo@zf.com')} className="p-2 bg-slate-950 border border-slate-800 rounded hover:border-cyan-500 text-slate-400 hover:text-cyan-400 transition-colors flex items-center justify-center gap-2">
                ⚡ Admin
              </button>
              <button onClick={() => handleQuickLogin('isabela.galvez@zf.com')} className="p-2 bg-slate-950 border border-slate-800 rounded hover:border-cyan-500 text-slate-400 hover:text-cyan-400 transition-colors flex items-center justify-center gap-2">
                📋 Auditor
              </button>
