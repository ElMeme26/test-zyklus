...
// ── UTF-8 BOM helper para que Excel abra CSVs con tildes correctamente ──
const BOM = '\uFEFF';
...
// ═══════════════════════════════════════════════════════════════
// 📊 EXPORTAR REQUESTS A EXCEL  (con codificación correcta)
// ═══════════════════════════════════════════════════════════════
export const exportRequestsToExcel = (requests: Request[]) => {
  ...
  // writeFile con type: 'buffer' + codificación correcta
  XLSX.writeFile(workbook, `zyklus_solicitudes_${format(new Date(), 'yyyy-MM-dd')}.xlsx`, { bookType: 'xlsx', type: 'file' });
};
...
export const exportRequestsByUser = (requests: Request[]) => {
  ...
  const byUser: Record<string, { nombre: string; disciplina: string; total: number; activos: number; devueltos: number; vencidos: number; dias_promedio: number; _dias: number[] }> = {};
  ...
  for (const r of requests) {
    ...
    if (!byUser[uid]) {
      byUser[uid] = {
        ...
        disciplina: r.requester_disciplina || '—',
        total: 0, activos: 0, devueltos: 0, vencidos: 0,
        dias_promedio: 0, _dias: [],
      };
    }
    byUser[uid].total++;
  ...
  const userSummary = Object.values(byUser).map(u => ({
    ...
    'Vencidos': u.vencidos,
    'Promedio Dias Prestamo': u._dias.length ? Math.round(u._dias.reduce((a, b) => a + b, 0) / u._dias.length) : 0,
  ...
  const disciplinaData = Object.entries(byDisciplina).map(([d, v]) => ({
    ...
    'Vencidos': v.vencidos,
    'Porcentaje del Total': `${Math.round((v.total / requests.length) * 100)}%`,
  })).sort((a, b) => b['Total Solicitudes'] - a['Total Solicitudes']);
...
export const exportMaintenanceReport = (maintenanceLogs: MaintenanceLog[], assets: Asset[]) => {
  ...
  const logsData = maintenanceLogs.map(log => ({
    ...
    'Estado': log.status === 'RESOLVED' ? 'Resuelto' : log.status === 'IN_PROGRESS' ? 'En Proceso' : 'Abierto',
    'Reportado Por': log.users?.name || '—',
    'Fecha Reporte': safeDate(log.created_at),
    'Fecha Resolucion': safeDate(log.resolved_at),
    'Costo Reparacion': log.cost ? `$${log.cost}` : '—',
    'Dias Resolucion': log.resolved_at
  ...
  for (const log of maintenanceLogs) {
    ...
    if (log.cost) byAsset[aid].costo += log.cost;
  }
  ...
  const needsMaint = assets.filter(a =>
    a.maintenance_alert || a.status === 'En mantenimiento' || a.status === 'Requiere Mantenimiento'
  ).map(a => ({
    'Tag': a.tag,
    'Nombre': a.name,
    ...
    'Proximo Mantenimiento': safeDate(a.next_maintenance_date),
    'Ubicacion': a.location || '—',
  }));

  const ws3 = XLSX.utils.json_to_sheet(needsMaint.length ? needsMaint : [{ 'Mensaje': 'No hay activos que requieran mantenimiento actualmente' }]);
  ws3['!cols'] = [
